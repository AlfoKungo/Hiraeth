...
namespace hiraeth {
	namespace input {
		...
		private:
			...
			KeyboardEvent* m_TypingKeyboardEvent{nullptr};
		...
		public:
			...
			void setTyping(KeyboardEvent* keyboard_event, bool is_typing);
		...
		private:
			...
			{
				if (m_TypingKeyboardEvent == nullptr)
				{
				...
				else
				{
					if (action == GLFW_PRESS)
						if (keycode >= 256 && keycode <= 270)
							m_TypingKeyboardEvent->ButtonClicked(keycode);
				}
			}
			...
			void char_input(unsigned int codepoint)
			{
				if (m_TypingKeyboardEvent != nullptr)
				{
					m_TypingKeyboardEvent->ButtonClicked(codepoint);
				}
			}
		};
