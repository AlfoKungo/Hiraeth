...
namespace hiraeth {
	namespace ui {
			...
			m_HoldItem(nullptr)
		{
			...
			m_Items.insert(std::pair<SRL::Tab, std::unique_ptr<graphics::Group>>(SRL::Tab::Equip, new graphics::Group()));
			m_Items.insert(std::pair<SRL::Tab, std::unique_ptr<graphics::Group>>(SRL::Tab::Use, new graphics::Group()));
			m_Items.insert(std::pair<SRL::Tab, std::unique_ptr<graphics::Group>>(SRL::Tab::SetUp, new graphics::Group()));
			m_Items.insert(std::pair<SRL::Tab, std::unique_ptr<graphics::Group>>(SRL::Tab::Etc, new graphics::Group()));
			m_Items.insert(std::pair<SRL::Tab, std::unique_ptr<graphics::Group>>(SRL::Tab::Cash, new graphics::Group()));
			...
			fillGroup();
			for (int tab_index = SRL::Tab::Equip; tab_index != SRL::Tab::Cash + 1; ++tab_index)
			{
				SRL::Tab tab = static_cast<SRL::Tab>(tab_index);
		...
		void UiInventory::mouse_clicked(maths::vec2 mousePos)
		{
			...
			{
				 m_Tab = static_cast<SRL::Tab>(unsigned int((mousePos.x -11) / 30));
				 fillGroup();
			}
			else
			{
