...
void serialize_generic(const std::string SRL_PATH, F fun, int DataBegin, int AddressBegin)
{
	...
	const std::string path = "./serialized/" + srl_type + ".data";
	if (!fs::exists(path))
	{
...
int main()
{
	const std::filesystem::path pathToDelete("./serialized");
	...
	std::string path_from = "./serialized/";
	fs::path server_path = "../Server/serialized/";
	fs::path hiraeth_path = "../Hiraeth/serialized/";
	remove_all(server_path);
	create_directories(server_path);
	remove_all(hiraeth_path);
	create_directories(hiraeth_path);
	...
	{
		fs::path sourceFile = entry.path();
		auto target = hiraeth_path / sourceFile.filename(); // sourceFile.filename() returns "sourceFile.ext".
		fs::copy_file(sourceFile, target, fs::copy_options::overwrite_existing);
	}
